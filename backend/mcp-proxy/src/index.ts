...
import type { Config, ToolMapping } from './types.js';

// ── Args ──────────────────────────────────────────────────────────────────────

function parseArgs(): { configPath: string; targetCmd: string } {
  ...
  if (configIdx === -1 || targetIdx === -1) {
    process.stderr.write(
      'Usage: mcp-proxy --config <rules.json> --target "<mcp server command>"\n\n' +
      'Example:\n' +
...
function findMapping(config: Config, toolName: string): ToolMapping | null {
...
function errorResponse(id: number | string | null, message: string): string {
...
function deniedResponse(id: number | string | null, toolName: string, reason: string): string {
  const res: JsonRpcResponse = {
    jsonrpc: '2.0',
    id,
    result: {
