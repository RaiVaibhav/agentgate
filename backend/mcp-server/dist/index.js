...
if (!SESSION_TOKEN) {
    process.stderr.write('[agent-permission-mcp] ERROR: SESSION_TOKEN env var is required.\n' +
        'Create a session in the dashboard at /agents/sessions and set SESSION_TOKEN=<token>\n');
    process.exit(1);
}
async function checkPermission(resourceType, resourcePath, action) {
...
// ── Tool: read_file ───────────────────────────────────────────────────────────
server.tool('read_file', 'Read the contents of a file. Permission is checked before reading — sensitive files like .env will be blocked.', {
    filePath: z.string().describe('Path to the file, relative to project root (e.g. src/index.ts) or absolute'),
}, async ({ filePath }) => {
...
// ── Tool: list_files ──────────────────────────────────────────────────────────
server.tool('list_files', 'List files in a directory. Permission is checked against the directory path.', {
    dirPath: z.string().describe('Directory path, relative to project root (e.g. src/) or absolute'),
}, async ({ dirPath }) => {
...
// ── Tool: call_endpoint ───────────────────────────────────────────────────────
server.tool('call_endpoint', 'Make an HTTP request to the dummy app API. Permission is checked before the request is sent.', {
